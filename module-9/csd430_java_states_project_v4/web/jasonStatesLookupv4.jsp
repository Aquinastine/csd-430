...
<%
    ...
    String currentPage = "jasonStatesLookupv4.jsp";
    String message = "";
    ...
    // Handle create, populate, drop, insert, display, update, and delete commands.
    if (action != null) {
        if ("create".equals(action)) {
            ...
            if (selectedIdText == null || selectedIdText.trim().length() == 0) {
                message = "Delete failed: Please select a record before deleting.";
            } else {
                try {
                    int stateId = Integer.parseInt(selectedIdText);

                    boolean deleted = statesBean.deleteStateById(stateId);

                    if (deleted) {
                        message = "The selected state record was deleted successfully.";
                        showTableCard = true;

                        // Clear the selected ID so the page does not try to display a deleted record.
                        selectedIdText = null;
                    } else {
                        message = "Delete failed: No matching record was found.";
                    }

                } catch (NumberFormatException e) {
                    message = "Delete failed: State ID must be a valid number.";
                }
            }
        }
    }
    ...
    // Do not fetch a selected record after a delete action because that record should no longer exist.
    if (selectedIdText != null
            && selectedIdText.trim().length() > 0
...
<body>
    ...
    <div class="page-wrapper">
        ...
        <div class="card">
            ...
            <h1 class="title">Module 10 Assignment - Deleting Existing Records</h1>
            <p class="subtitle">Jason States Data</p>
            <p>
            This JSP page uses a JavaBean and JDBC to create, read, update, and delete data from the
            ...
            <div class="button-row">
                ...
                <form method="post" action="<%= currentPage %>">
                    ...
                    <button type="submit" class="danger"
                        onclick="return confirm('Are you sure you want to drop the whole table? This cannot be undone.');">
                        Drop Table
                    </button>
                </form>
            ...
            <% } else { %>
                <form method="post" action="<%= currentPage %>">
                    ...
                    <button type="submit" name="action" value="delete" class="danger"
                        onclick="return confirm('Are you sure you want to delete the selected record? This cannot be undone.');">
        ...
        <!-- Normally hidden table results card, is unhidden after insert, update, populate, or delete. -->
        <% if (showTableCard) { %>
        <div class="card">
            <h2>All State Records</h2>
            <p class="muted">The table below shows the current records after the requested action.</p>
        ...
        <div class="card">
            <h1 class="subtitle">Module 10 Assignment - Delete Existing Record</h1>
            <p class="footer-name">Jason Luttrell</p>
            <p class="footer-date">May 10, 2026</p>
        </div>
    </div>
</body>
</html>